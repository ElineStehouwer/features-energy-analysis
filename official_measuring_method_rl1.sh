...
for ((i=1; i<=$num_runs; i++)); do
    ...
    then 
        executable_path="Rosetta/100-doors/100-doors.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 61200
    fi
    if [ "$section_to_execute" -eq 2 ]
    then 
        executable_path="Rosetta/100-doors/100-doors-bc.gcc_run"
        echo "$executable_path" 
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 61200
    fi
    if [ "$section_to_execute" -eq 3 ]
    then 
        executable_path="Rosetta/100-doors/dynamic-typing/100-doors-dt"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 25200
    fi
    if [ "$section_to_execute" -eq 4 ]
    then 
        executable_path="Rosetta/Avl-tree/use-avl-tree.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 7200
    fi
    if [ "$section_to_execute" -eq 5 ]
    then
        executable_path="Rosetta/Binary-digits/binary-digits.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 51200
    fi
    if [ "$section_to_execute" -eq 6 ]
    then 
        executable_path="Rosetta/Binary-digits/binary-digits-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 51200
    fi
    if [ "$section_to_execute" -eq 7 ]
    then 
        executable_path="Rosetta/Binary-digits/dynamic-typing/binary-digits-dt"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 12000
    fi
    if [ "$section_to_execute" -eq 8 ]
    then 
        executable_path="Rosetta/Factors-of-an-integer/factors-of-an-integer.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 51200
    fi
    if [ "$section_to_execute" -eq 9 ]
    then
        executable_path="Rosetta/Factors-of-an-integer/factors-of-an-integer-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 51200
    fi
    if [ "$section_to_execute" -eq 10 ]
    then 
        executable_path="Rosetta/Hailstone-sequence/hailstone-sequence.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 1950
    fi
    if [ "$section_to_execute" -eq 11 ]
    then 
        executable_path="Rosetta/Hailstone-sequence/hailstone-sequence-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 1950
    fi
    if [ "$section_to_execute" -eq 12 ]
    then 
        executable_path="Rosetta/N-queens-problem/n-queens-problem.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 7200
    fi
    if [ "$section_to_execute" -eq 13 ]
    then
        executable_path="Rosetta/N-queens-problem/n-queens-problem-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 7200
    fi
    if [ "$section_to_execute" -eq 14 ]
    then 
        executable_path="Rosetta/N-queens-problem/dynamic-typing/n-queens-problem-dt"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 250
    fi
    if [ "$section_to_execute" -eq 15 ]
    then     
        executable_path="Rosetta/Remove-duplicate-elements/remove-duplicate-elements.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 2600 
    fi
    if [ "$section_to_execute" -eq 16 ]
    then 
        executable_path="Rosetta/Remove-duplicate-elements/remove-duplicate-elements-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 2600
    fi
    if [ "$section_to_execute" -eq 17 ]
    then
        executable_path="Rosetta/Sieve-of-Eratosthenes/sieve-of-eratosthenes.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 61200
    fi
    if [ "$section_to_execute" -eq 18 ]
    then 
        executable_path="Rosetta/Sieve-of-Eratosthenes/sieve-of-eratosthenes-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 61200
    fi
    if [ "$section_to_execute" -eq 19 ]
    then     
        executable_path="Rosetta/Sieve-of-Eratosthenes/dynamic-typing/sieve-of-eratosthenes-dt"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 4000
    fi
    if [ "$section_to_execute" -eq 20 ]
    then 
        executable_path="Rosetta/Sorting-algorithms-Merge-sort/sorting-algorithms-merge-sort.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 22000
    fi
    if [ "$section_to_execute" -eq 21 ]
    then
        executable_path="Rosetta/Sorting-algorithms-Merge-sort/sorting-algorithms-merge-sort-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 22000
    fi
    if [ "$section_to_execute" -eq 22 ]
    then 
        executable_path="Rosetta/Sorting-algorithms-Merge-sort/dynamic-typing/sorting-algorithms-merge-sort-dt"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 1300
    fi    
    if [ "$section_to_execute" -eq 23 ]
    then     
        executable_path="Rosetta/Sorting-algorithms-Quicksort/sorting-algorithms-quick-sort.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 26200
    fi
    if [ "$section_to_execute" -eq 24 ]
    then 
        executable_path="Rosetta/Sorting-algorithms-Quicksort/sorting-algorithms-quick-sort-bc.gcc_run"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 26200
    fi
    if [ "$section_to_execute" -eq 25 ]
    then 
        executable_path="Rosetta/Sorting-algorithms-Quicksort/dynamic-typing/sorting-algorithms-quick-sort-dt"
        echo "$executable_path"
        get_program_power_and_time_consumption1 "$executable_path" "$num_runs" 1800
    fi
    echo "================================NEW RUN================================"
done